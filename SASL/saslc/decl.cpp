...
printQual( std::ostream& out, TypeQual qualifier )
{
	...
	if ((qualifier & TQ_NoInteroperation) != 0)
		out << "nointeroperation ";

	if ((qualifier & TQ_Shared) != 0)
		out << "shared ";

	if ((qualifier & TQ_Uniform) != 0)
		out << "uniform ";
}
...
void BaseType::printSize(std::ostream& out, int level) const
{
	unsigned int size1d = 0, size2d = 0;
	MatrixSize(typemask, size1d, size2d);
	if(IsVectorType(typemask)){
		out << size1d;
	} else if (IsMatrxiType(typemask)){
		out << size1d << 'x' << size2d;
	}
}
...
void BaseType::printBase(std::ostream& out, int level) const
{
    ...
    if (GetBaseType(typemask) == BT_Void)
        out << "void";
    else if (GetBaseType(typemask) == BT_Int)
        out << "int";
    else if (GetBaseType(typemask) == BT_UInt)
        out << "uint";//FIXME
    else if (GetBaseType(typemask) == BT_Half)
        out << "half";
	else if (GetBaseType(typemask) == BT_Float)
		out << "float";
	else if (GetBaseType(typemask) == BT_Double)
		out << "double";
	else if (GetBaseType(typemask) == BT_Boolean)
		out << "bool";
    else if (GetBaseType(typemask) == BT_Struct)
    {
        if (stDefn != NULL)
        {
    ...
    else if (GetBaseType(typemask) == BT_UserType)
    {
        if (typeName)
            out << *typeName << " ";
    }
