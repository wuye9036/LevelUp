...
{
public:
	const std::vector<h_compile_flag>& get_flags() const;
	std::vector<h_compile_flag>& get_flags();

	virtual compile_flag::flag_types get_type() const;
private:
	std::vector<h_compile_flag> flags_;
};
...
public:
	...
	static h_makefile from_makefile(const std::string& file);
	static h_makefile from_directory(const std::string& dir);
