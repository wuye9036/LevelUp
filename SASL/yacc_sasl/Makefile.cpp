...
const vector<h_compile_flag> & compile_flags::get_flags() const
{
	return flags_;
}

vector<h_compile_flag> & compile_flags::get_flags()
{
	return flags_;
}
/*********
...
public:
	...
	const TiXmlElement* get_result_element(){
		return pelem;
	}
...
bool configuration_from_xmlelem(configuration& cfg, TiXmlElement& elem)
{
	...
	for (TiXmlNode* pnode = elem.FirstChild(); pnode != NULL; pnode = pnode->NextSibling())
	{
		...
		if ( pElem->ValueStr() == string("work_env") ){
		...
		if ( pElem->ValueStr() == string("compile_flags") ){
...
bool compile_flags_from_xmlelem(compile_flags& flags, TiXmlElement& elem)
{
	if (elem.ValueStr() != string("compile_flags") )
	{
		return false;
	}
	...
	for (TiXmlNode* pnode = elem.FirstChild(); pnode != NULL; pnode = pnode->NextSibling())
	{
		...
		if ( pElem->ValueStr() == string("Flag") ){
			flags.get_flags().push_back(h_compile_flag(new compile_flags()));
		...
		if ( pElem->ValueStr() == string("FlagGroup") ){
			flags.Flags().push_back(h_compile_flag(new compile_flags()));
			if ( !compile_flags_from_xmlelem(*((compile_flags*)(flags.get_flags().back().get())), *pElem) )
			{
				flags.Flags().pop_back();
				return false;
			}
			continue;
		}
...
makefile
**/
h_makefile makefile::from_makefile(const string& name)
{
...
h_makefile makefile::from_directory(const string& name)
{
