...
def CreateSensViewerModel(modelName):
	if modelName == "WSM":
		return WSMSensViewerModel()
	if modelName == "WPM":
		return WPMSensViewerModel()
	
class SensViewerModel(QtCore.QAbstractTableModel):
	...
	def data(self, index, role):
		...
		if 1 < col and col < crtCount + 2:
			if not self.sens_[row][col-2]: return QtCore.QVariant("None")
...
class WSMSensViewerModel(SensViewerModel):
	def __init__(self, parent = None):
		SensViewerModel.__init__(self, parent)
	...
	def _LoadChgMatAndSens(self, altCrtMat):
		...
		altSens.Compute()
		self.sens_ = altSens.AlternativeSensitives()
		
	def Rank2Idx(self, rank):
		return self.altCrtMat_.IdxByRank(rank)

class WPMSensViewerModel(SensViewerModel):
	def __init__(self, parent = None):
		SensViewerModel.__init__(self, parent)
	...
	def _LoadChgMatAndSens(self, altCrtMat):
		...
		altSens.Compute()
		self.sens_ = altSens.AlternativeSensitives()
		self.ratioTable_ = self.chgMat_.RatioTable()
		
	def Rank2Idx(self, rank):
		return self.ratioTable_.Rank2Idx(rank)